...
(function() { // Add this
  ...
  ProductGridItem.prototype.compileTemplate = function(data, index) {
    ...
    // Add 11% discount
    //         var tagForSale = data.tags.findIndex(item => item == "sitewide-sale")

    // itemHtml = itemHtml.replace(/{{itemDiscountLabel}}/g, itemDiscountLabel())

    // function itemDiscountLabel() {
    //   var html = '';
      
    // if(tagForSale != -1)  html += '<span class="discount_Label">11% OFF</span>'
    ...
    itemProductInfoHtml += '{{itemShippingImage}}';
    ...
    if (typeof comingsoonLabel[0] !== 'undefined') {
      ...
      if (!soldOut || boostPFSConfig.custom.display_price) {
        //console.log(data);
        var strMain = data.title;
        var str1 = "Mix and Match";
        var str2 = "2";
        var str3 = "4";
        var str4 = "6";
        if(strMain.indexOf(str1) != -1 && strMain.indexOf(str2) != -1){
          bottleCount = 2;
          bundlePer = 5;
        }
        if(strMain.indexOf(str1) != -1 && strMain.indexOf(str3) != -1){
          bottleCount = 4;
          bundlePer = 10;
        }
        if(strMain.indexOf(str1) != -1 && strMain.indexOf(str4) != -1){
          bottleCount = 6;
          bundlePer = 15;
        }
        //console.log(bottleCount);
        itemPriceHtml += '<span class="current_price '+bunldeclass+'" data-price-without="'+data.price_max*bottleCount+'" data-price-per="'+bundlePer+'">';
        if (priceVaries && data.price_min > 0) {
          itemPriceHtml += '<small><em>' + boostPFSConfig.label.from_price + '</em></small> ';
        }
        // Sale Code - Change the number here.
        // if (data.price_min > 0) {
        //   var originalPrice = data.price_min;
        //   var discount = ((11 * data.price_min) / 100).toFixed(2)
        //   var newPrice = originalPrice - discount

        //   if(tagForSale == -1) {
        //     newPrice = data.price_min
        //   }
        // Sale Code Ends
        if (data.price_min > 0) {
      ...
      if (soldOut) {
        itemPriceHtml += boostPFSConfig.label.sold_out;
      }
      itemPriceHtml += ' </div>';
    }
    ...
    var shippingImageHtml = "<img src='"+ imageSrc +"' class='boost-shipping-image' >"
    var productShippingImage = data.handle.indexOf("ready-to-stock") > -1 ? shippingImageHtml : ''
    itemHtml = itemHtml.replace(/{{itemShippingImage}}/g , productShippingImage)
    ...
    FilterApi.beforeCall = function(eventType, eventInfo) {
      ...
      if(Utils.isSearchPage()){
      	if(typeof(localStorage.getItem("boostSearchCollection")) !== 'undefined' && localStorage.getItem("boostSearchCollection") !== null){
        	Globals.queryParams.collection_scope = localStorage.getItem("boostSearchCollection");
        }else{
        	var defaultCollectionVal = jQ('.bc-select-collection').val();
          	Globals.queryParams.collection_scope = defaultCollectionVal;
        }
      }
